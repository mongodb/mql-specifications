...
Adding the `mql-spec` label to a `DRIVERS-*` Jira ticket automatically generates a branch and draft PR with the YAML spec, fetched from MongoDB documentation sources. Submitting **Request changes** on an automated PR automatically applies the review corrections and pushes them to the branch.
